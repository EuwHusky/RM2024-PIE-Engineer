...
// 裁判系统串口初始化
static void referee_uart_init(void);
// DMA接收&发送触发函数
static hpm_stat_t uart_rx_trigger_dma(DMA_Type *dma_ptr, uint8_t ch_num, UART_Type *uart_ptr, uint32_t dst, uint32_t size);
static hpm_stat_t uart_tx_trigger_dma(DMA_Type *dma_ptr, uint8_t ch_num, UART_Type *uart_ptr, uint32_t src, uint32_t size);
// 裁判系统数据解包
static void referee_unpack_fifo_data(void);
// 自定义UI相关操作
// static void UI_Delate(uint8_t Del_Operate, uint8_t Del_Layer);
static void Line_Draw(graphic_data_struct_t *image, char imagename[3], uint32_t Graph_Operate, uint32_t Graph_Color, uint32_t Start_x, uint32_t Start_y, uint32_t End_x, uint32_t End_y);
static void Rectangle_Draw(graphic_data_struct_t *image, char imagename[3], uint32_t Graph_Operate, uint32_t Graph_Color, uint32_t Start_x, uint32_t Start_y, uint32_t End_x, uint32_t End_y);
static void Circle_Draw(graphic_data_struct_t *image, char imagename[3], uint32_t Graph_Operate, uint32_t Graph_Color, uint32_t Start_x, uint32_t Start_y, uint32_t Graph_Radius);
static void Arc_Draw(graphic_data_struct_t *image, char imagename[3], uint32_t Graph_Operate, uint32_t Graph_Color, uint32_t start_angle, uint32_t end_angle, uint32_t Start_x, uint32_t Start_y, uint32_t Graph_Radius, uint32_t width);
static void Float_Draw(graphic_data_struct_t *image, char imagename[3], uint32_t Graph_Operate, uint32_t Graph_Color, uint32_t Start_x, uint32_t Start_y, int32_t Graph_Float);
static void Char_Draw(ext_client_custom_character_t *image, char imagename[3], uint32_t Graph_Operate, uint32_t Graph_Color, uint32_t Start_x, uint32_t Start_y, char *Char_Data, uint32_t Graph_Size);
...
/*----------------------------------------*/
// 接受裁判系统数据相关变量和结构
ATTR_PLACE_AT_NONCACHEABLE uint8_t referee_rx_buf[USART_RX_BUF_LENGHT]; // 接收原始数据
volatile bool uart_rx_dma_done = true;                                  // dma传输完成标志位
unpack_data_t referee_unpack_obj;                                       // 数据解包结构体
fifo_s_t referee_fifo;                                                  // FIFO结构体
uint8_t referee_fifo_buf[REFEREE_FIFO_BUF_LENGTH];                      // FIFO缓存数组

static Refree_data_t refree_data; // 需要用到的裁判系统数据结构体
/*----------------------------------------*/
uint16_t Robot_ID;
uint16_t Cilent_ID;
...
void Referee_task(void *pvParameters) {
  ...
  referee_uart_init();                                                   // 初始化裁判系统串口
  init_referee_struct_data();                                            // 初始化裁判系统数据结构体
  fifo_s_init(&referee_fifo, referee_fifo_buf, REFEREE_FIFO_BUF_LENGTH); // FIFO初始化

  while (1) {
    if (uart_rx_dma_done) {
      uart_rx_dma_done = false;
      referee_unpack_fifo_data();
      uart_rx_trigger_dma(BOARD_HDMA, REFEREE_UART_RX_DMA_CHN, REFEREE_UART,
          core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (uint32_t)referee_rx_buf), USART_RX_BUF_LENGHT);
    }
...
void dma_isr(void) {
  ...
  stat_rx_chn = dma_check_transfer_status(BOARD_HDMA, REFEREE_UART_RX_DMA_CHN);
  if (stat_rx_chn & DMA_CHANNEL_STATUS_TC) {
    fifo_s_puts(&referee_fifo, (char *)referee_rx_buf, USART_RX_BUF_LENGHT);
    uart_rx_dma_done = true; // 更新标志位
    detect_hook(My_referee);
  }
}
...
void referee_uart_init(void) {
  ...
  board_init_uart(REFEREE_UART);
  uart_default_config(REFEREE_UART, &config);                    // 填充默认配置
  config.fifo_enable = true;                                     // 使能FIFO
  config.dma_enable = true;                                      // 使能DMA
  config.baudrate = REFEREE_BAUDRATE;                            // 设置波特率
  config.src_freq_in_hz = clock_get_frequency(REFEREE_UART_CLK); // 获得时钟频率
  config.rx_fifo_level = uart_rx_fifo_trg_not_empty;
  if (uart_init(REFEREE_UART, &config) != status_success) {
    printf("failed to initialize uart\n");
    while (1) {
    }
  }
  intc_m_enable_irq_with_priority(BOARD_HDMA_IRQ, 1);
  dmamux_config(BOARD_DMAMUX, REFEREE_UART_RX_DMAMUX_CHN, REFEREE_UART_RX_DMA_REQ, true);
}

Refree_data_t *get_refree_data_point(void) {
  return &refree_data;
}
...
int UI_ReFresh_char(ext_client_custom_character_t *char_data) // UI字符型刷新推送
{
  ...
  memcpy(framepoint + sizeof(ui_packhead) + sizeof(ui_id), framepoint2, sizeof(char_data->grapic_data_struct)); // 填充图形操作信息

  framepoint2 = (unsigned char *)&char_data->data;
  memcpy(framepoint + sizeof(ui_packhead) + sizeof(ui_id) + sizeof(char_data->grapic_data_struct), framepoint2, sizeof(char_data->data)); // 填充图形操作信息
...
int UI_ReFresh_picture(Refreedata_UI *refreedata_UI) {
  ...
  for (uint8_t i = 0; i < refreedata_UI->num; i++)
    memcpy(refreedata_UI->Picture_Data + sizeof(ui_packhead) + sizeof(ui_id) + i * sizeof(graphic_data_struct_t), refreedata_UI->graphic_data[i], sizeof(graphic_data_struct_t)); // 填充图形操作信息
...
**参数:*image Graph_Data 类型变量指针，用于存放数据
        ...
        End_x,End_y      中止坐标
**********************************************************************************************************/
void Line_Draw(graphic_data_struct_t *image, char imagename[3], uint32_t Graph_Operate, uint32_t Graph_Color, uint32_t Start_x, uint32_t Start_y, uint32_t End_x, uint32_t End_y) {
  int i;
...
**********************************************************************************************************/
void Rectangle_Draw(graphic_data_struct_t *image, char imagename[3], uint32_t Graph_Operate, uint32_t Graph_Color, uint32_t Start_x, uint32_t End_x, uint32_t Start_y, uint32_t End_y) {
  int i;
  for (i = 0; i < 3 && imagename[i] != '\0'; i++)
    image->graphic_name[2 - i] = imagename[i];
...
**参数：*image Graph_Data类型变量指针，用于存放图形数据
        ...
        Graph_Radius  图形半径
**********************************************************************************************************/
void Circle_Draw(graphic_data_struct_t *image, char imagename[3], uint32_t Graph_Operate, uint32_t Graph_Color, uint32_t Start_x, uint32_t Start_y, uint32_t Graph_Radius) {
  int i;
  for (i = 0; i < 3 && imagename[i] != '\0'; i++)
    image->graphic_name[2 - i] = imagename[i];
...
**参数：*image Graph_Data类型变量指针，用于存放图形数据
        ...
        Graph_Radius  圆弧半径
**********************************************************************************************************/
void Arc_Draw(graphic_data_struct_t *image, char imagename[3], uint32_t Graph_Operate, uint32_t Graph_Color, uint32_t start_angle, uint32_t end_angle, uint32_t Start_x, uint32_t Start_y, uint32_t Graph_Radius, uint32_t width) {
  int i;
  for (i = 0; i < 3 && imagename[i] != '\0'; i++)
    image->graphic_name[2 - i] = imagename[i];
...
**********************************************************************************************************/
void Char_Draw(ext_client_custom_character_t *image, char imagename[3], uint32_t Graph_Operate, uint32_t Graph_Color, uint32_t Start_x, uint32_t Start_y, char *Char_Data, uint32_t Graph_Size) {
  int i;
...
**参数：*image Graph_Data类型变量指针，用于存放图形数据
        ...
        Graph_Float   要显示的变量
**********************************************************************************************************/
void Float_Draw(graphic_data_struct_t *image, char imagename[3], uint32_t Graph_Operate, uint32_t Graph_Color, uint32_t Start_x, uint32_t Start_y, int32_t Graph_Float) {
  int i;
