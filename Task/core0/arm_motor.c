 ...
 * @brief 初始化机械臂电机
 */
void arm_motor_init(engineer_scara_arm_s *arm)
{
    ...
    config.external_angle = NULL;
    config.can_ordinal = ENGINEER_ARM_JOINTS_123_MOTORS_CAN_ORDINAL;
    config.master_can_id = 0x205;
    rflMotorInit(arm->joint_1_motor + 0, &config);

    config.is_reversed = false;
    config.external_angle = NULL;
    config.can_ordinal = ENGINEER_ARM_JOINTS_123_MOTORS_CAN_ORDINAL;
    config.master_can_id = 0x206;
    rflMotorInit(arm->joint_1_motor + 1, &config);
    ...
    rflMotorInit(arm->joint_23_motor + 0, &config);
    ...
    rflMotorInit(arm->joint_23_motor + 1, &config);
    ...
    rflMotorInit(arm->joint_4_motor + 0, &config);
    ...
    rflMotorInit(arm->joint_56_motor + 0, &config);
    ...
    rflMotorInit(arm->joint_56_motor + 1, &config);
}

/**
 * @brief 更新机械臂电机状态量并执行控制
 */
void arm_motor_update_and_execute(engineer_scara_arm_s *arm)
{
    // 设定电机角度
    if (arm->mode != ARM_MODE_STARTING)
    {
        rflMotorSetAngle(arm->joint_1_motor + 0, RFL_ANGLE_FORMAT_DEGREE,
                         arm->set_joints_value[0] * ENGINEER_ARM_JOINT_1_DISTANCE_TO_ANGLE_FACTOR);
        rflMotorSetAngle(arm->joint_1_motor + 1, RFL_ANGLE_FORMAT_DEGREE,
                         arm->set_joints_value[0] * ENGINEER_ARM_JOINT_1_DISTANCE_TO_ANGLE_FACTOR);

        rflMotorSetAngle(arm->joint_23_motor + 0, RFL_ANGLE_FORMAT_RADIAN, arm->set_joints_value[1]);

        rflMotorSetAngle(arm->joint_23_motor + 1, RFL_ANGLE_FORMAT_RADIAN, arm->set_joints_value[2]);

        rflMotorSetAngle(arm->joint_4_motor + 0, RFL_ANGLE_FORMAT_RADIAN, arm->set_joints_value[3]);

        rflMotorSetAngle(arm->joint_56_motor + 0, RFL_ANGLE_FORMAT_RADIAN,
                         ((arm->set_joints_value[4] * EFFECTOR_TIMING_BELT_TRANSMISSION_RATIO) -
                          (arm->set_joints_value[5] * EFFECTOR_TIMING_BELT_TRANSMISSION_RATIO *
                           EFFECTOR_CONICAL_TOOTH_PAIR_TRANSMISSION_RATIO)) *
                             0.5f);
        rflMotorSetAngle(arm->joint_56_motor + 1, RFL_ANGLE_FORMAT_RADIAN,
                         ((arm->set_joints_value[4] * EFFECTOR_TIMING_BELT_TRANSMISSION_RATIO) +
                          (arm->set_joints_value[5] * EFFECTOR_TIMING_BELT_TRANSMISSION_RATIO *
                           EFFECTOR_CONICAL_TOOTH_PAIR_TRANSMISSION_RATIO)) *
                             0.5f);
    ...
    // 电机更新&执行
    rflMotorUpdateStatus(arm->joint_1_motor + 0);
    rflMotorUpdateStatus(arm->joint_1_motor + 1);
    rflMotorUpdateControl(arm->joint_1_motor + 0);
    rflMotorUpdateControl(arm->joint_1_motor + 1);

    rflRmMotorControl(ENGINEER_ARM_JOINTS_123_MOTORS_CAN_ORDINAL, ENGINEER_ARM_JOINTS_123_RM_MOTORS_CAN_SLAVE_ID,
                      (int16_t)rflMotorGetOutput(arm->joint_1_motor + 0),
                      (int16_t)rflMotorGetOutput(arm->joint_1_motor + 1), 0, 0);

    rflMotorUpdateStatus(arm->joint_23_motor + 0);
    rflMotorUpdateControl(arm->joint_23_motor + 0);
    rflMotorExecuteControl(arm->joint_23_motor + 0);

    rflMotorUpdateStatus(arm->joint_23_motor + 1);
    rflMotorUpdateControl(arm->joint_23_motor + 1);
    rflMotorExecuteControl(arm->joint_23_motor + 1);

    rflMotorUpdateStatus(arm->joint_4_motor + 0);
    rflMotorUpdateControl(arm->joint_4_motor + 0);

    rflMotorUpdateStatus(arm->joint_56_motor + 0);
    rflMotorUpdateStatus(arm->joint_56_motor + 1);
    rflMotorUpdateControl(arm->joint_56_motor + 0);
    rflMotorUpdateControl(arm->joint_56_motor + 1);

    rflRmMotorControl(ENGINEER_ARM_JOINTS_456_MOTORS_CAN_ORDINAL, ENGINEER_ARM_JOINTS_456_RM_MOTORS_CAN_SLAVE_ID,
                      (int16_t)rflMotorGetOutput(arm->joint_4_motor + 0),
                      (int16_t)rflMotorGetOutput(arm->joint_56_motor + 0),
                      (int16_t)rflMotorGetOutput(arm->joint_56_motor + 1), 0);
}

/**
 * @brief 设置机械臂电机控制模式
 */
void arm_motor_set_mode(engineer_scara_arm_s *arm, rfl_motor_control_mode_e mode)
{
    if (mode != RFL_MOTOR_CONTROL_MODE_NO_FORCE && mode != RFL_MOTOR_CONTROL_MODE_SPEED_ANGLE)
        return;

    rflMotorSetMode(arm->joint_1_motor + 0, mode);
    rflMotorSetMode(arm->joint_1_motor + 1, mode);
    rflMotorSetMode(arm->joint_23_motor + 0, mode);
    rflMotorSetMode(arm->joint_23_motor + 1, mode);
    rflMotorSetMode(arm->joint_4_motor + 0, mode);
    rflMotorSetMode(arm->joint_56_motor + 0, mode);
    rflMotorSetMode(arm->joint_56_motor + 1, mode);
}
 ...
 */
void arm_motor_set_max_speed(engineer_scara_arm_s *arm, float max_speed)
{
    rflMotorSetMaxSpeed(arm->joint_1_motor + 0, max_speed);
    rflMotorSetMaxSpeed(arm->joint_1_motor + 1, max_speed);
    rflMotorSetMaxSpeed(arm->joint_23_motor + 0, max_speed);
    rflMotorSetMaxSpeed(arm->joint_23_motor + 1, max_speed);
    rflMotorSetMaxSpeed(arm->joint_4_motor + 0, max_speed);
    rflMotorSetMaxSpeed(arm->joint_56_motor + 0, max_speed);
    rflMotorSetMaxSpeed(arm->joint_56_motor + 1, max_speed);
}

/**
 * @brief 设置机械臂电机可达角度
 */
void arm_motor_set_angle_limit(engineer_scara_arm_s *arm, arm_mode_e mode)
{
    if (mode == ARM_MODE_STARTING)
    {
        rflMotorSetDegAngleLimit(arm->joint_1_motor + 0, RFL_ANGLE_FORMAT_DEGREE,
                                 ENGINEER_ARM_JOINT_1_INITIAL_MAX_ANGLE, ENGINEER_ARM_JOINT_1_INITIAL_MIN_ANGLE);
        rflMotorSetDegAngleLimit(arm->joint_1_motor + 1, RFL_ANGLE_FORMAT_DEGREE,
                                 ENGINEER_ARM_JOINT_1_INITIAL_MAX_ANGLE, ENGINEER_ARM_JOINT_1_INITIAL_MIN_ANGLE);

        // rflMotorSetDegAngleLimit(arm->joint_56_motor + 0, RFL_ANGLE_FORMAT_DEGREE,
        //                          ENGINEER_ARM_JOINT_56_MOTOR_MAX_ANGLE, ENGINEER_ARM_JOINT_56_MOTOR_MIN_ANGLE);
        // rflMotorSetDegAngleLimit(arm->joint_56_motor + 1, RFL_ANGLE_FORMAT_DEGREE,
        //                          ENGINEER_ARM_JOINT_56_MOTOR_MAX_ANGLE, ENGINEER_ARM_JOINT_56_MOTOR_MIN_ANGLE);
    }
    else if (mode != ARM_MODE_STARTING)
    {
        rflMotorSetDegAngleLimit(arm->joint_1_motor + 0, RFL_ANGLE_FORMAT_DEGREE, ENGINEER_ARM_JOINT_1_MAX_ANGLE,
                                 ENGINEER_ARM_JOINT_1_MIN_ANGLE);
        rflMotorSetDegAngleLimit(arm->joint_1_motor + 1, RFL_ANGLE_FORMAT_DEGREE, ENGINEER_ARM_JOINT_1_MAX_ANGLE,
                                 ENGINEER_ARM_JOINT_1_MIN_ANGLE);

        rflMotorSetDegAngleLimit(arm->joint_23_motor + 0, RFL_ANGLE_FORMAT_DEGREE, ENGINEER_ARM_JOINT_2_MAX_ANGLE,
                                 ENGINEER_ARM_JOINT_2_MIN_ANGLE);

        rflMotorSetDegAngleLimit(arm->joint_23_motor + 1, RFL_ANGLE_FORMAT_DEGREE, ENGINEER_ARM_JOINT_3_MAX_ANGLE,
                                 ENGINEER_ARM_JOINT_3_MIN_ANGLE);

        rflMotorSetDegAngleLimit(arm->joint_4_motor, RFL_ANGLE_FORMAT_DEGREE, ENGINEER_ARM_JOINT_4_MAX_ANGLE,
